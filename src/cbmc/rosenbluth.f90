...
!!$if(mol == 13)write(6,*)StepNumber,RosenbluthWeight,overlap,__LINE__
      if(overlap)return
    end do
...
!!$        rosen(TrialNumber)=0._PR
!!$        if(existing_molecule)then
!!$          overlap=.true.                 !** Something wrong as existing atoms are overlapping
!!$          return
!!$        end if
!!$      else
        Success=.true.
...
!!$!!$        call ieee_set_flag(ieee_underflow,.false.)
!!$!!$        rosen(TrialNumber)=0._PR
!!$!!$        if(existing_molecule)then                      !** something wrong existing atoms overlapping
!!$!!$          overlap=.true.
!!$!!$          return
!!$!!$        end if
!!$!!$      else
...
!!$
  ...
  function BranchPointSampling(Step,sys,spc,mol,beta,existing_molecule,overlap) result(RosenbluthWeight)
    ...
    if(existing_molecule)then
      ...
      r3=sin(Theta(1))*cos(phi_trial(TorsionTrialNumber))*vecX+ &
         sin(Theta(1))*sin(phi_trial(TorsionTrialNumber))*vecY- &
         cos(Theta(1))*vecZ
    end if
    Torsion=Molecule_intra(spc)%Torsion(Step%TorsionNumber(1,1))
    a1=Molecule_Intra(spc)%TorsionAtom(Step%TorsionNumber(1,1),1)
    a4=Molecule_Intra(spc)%TorsionAtom(Step%TorsionNumber(1,1),4)
    if(Step%Atom(1) == a1)then
      Position_p3=TrialCoordinates(spc,sys)%Positions(:,mol,a4)
    else
      Position_p3=TrialCoordinates(spc,sys)%Positions(:,mol,a1)
    end if
    ...
    if(existing_molecule)then
      ...
      !** Compute Trial Position
      vec=sin(Theta(1))*cos(phi_B(i)+phi_T(TrialNumber))*vecX+ &
          sin(Theta(1))*sin(phi_B(i)+phi_T(TrialNumber))*vecY- &
          cos(Theta(1))*vecZ
      vec=Position_p1+vec*BondLength(i)
    end if
    !** Calculate Intermolecular Interactions
    ...
    do TrialNumber=2,NumberOfTrialInsertions
      ...
      energy=0._PR
      atm=Step%Atom(1)
      vec=sin(Theta(1))*cos(phi_T(TrialNumber))*vecX+ &
          sin(Theta(1))*sin(phi_T(TrialNumber))*vecY- &
          cos(Theta(1))*vecZ
      vec=Position_p1+vec*BondLength(i)
      !** Calculate Intermolecular Interactions
      call AtomSystemShortRangePairwiseInteraction(vec,vec,CurrentCoordinates(:,sys),CurrentSimulationCell(sys), &
  ...
  function ChosenTrialNumber(NumberOfTrials,rosen) result(TrialNumber)
    ...
    if(TrialNumber > NumberOfTrials)TrialNumber=1
